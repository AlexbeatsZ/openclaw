...
describe("agy provider", () => {
  ...
  it("registers a cli-backed provider and reads its model catalog from runtime config", async () => {
    ...
    const auth = provider.auth as Array<{
      run: (ctx: { config: OpenClawConfig }) => Promise<unknown>;
    }>;
    await expect(auth[0]?.run({ config: configuredAgyConfig })).resolves.toMatchObject({
  ...
  it("discovers with the same effective command, workspace, and env as the CLI backend", async () => {
    ...
    const normalizeConfig = cliBackend.normalizeConfig as (
      backendConfig: Record<string, unknown>,
      context: { config: OpenClawConfig },
    ...
    expect(normalizeConfig(cliBackend.config as Record<string, unknown>, { config })).toMatchObject(
      {
        command: "custom-agy",
        env: { AGY_PROFILE: "secondary" },
      },
    ...
    ).toEqual(["--model-id", "Gemini 9.1 Flash (High)", "--effort", "high", "--print", "{prompt}"]);
  });
  ...
  it("omits the CLI backend system prompt when configured", () => {
    const config = {
      plugins: { entries: { agy: { config: { systemPromptMode: "none" } } } },
    } satisfies OpenClawConfig;
    const { cliBackend } = collectProviderRegistration({}, config);
    const transformSystemPrompt = cliBackend.transformSystemPrompt as (ctx: {
      config: OpenClawConfig;
    ...
    expect(
      transformSystemPrompt({
        ...
        modelId: CONFIGURED_MODEL_ID,
        modelDisplay: "Gemini Flash",
        systemPrompt: "Do not forward this.",
      }),
    ).toBe("");
  });
