...
- **Professional Core** — A separate professional-work context with its own workspace, transcript, and memory. It uses the shared OpenClaw model catalog and runtime adapters, including the configured `agy` CLI backend.
...
- **Mode Switch** — A user-requested core change. It rotates the session lifecycle and context binding; it is never an in-place prompt/persona toggle.
...
- Professional prompt/bootstrap/memory construction reads only its isolated core workspace.
- Model discovery and execution are shared infrastructure, not shared context. Selecting an `agy` model uses the existing `agy` CLI authentication and runtime path.
- Cross-core forks are rejected. Same-core Professional forks preserve only Professional history.
