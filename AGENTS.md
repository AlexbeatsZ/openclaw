...
- This is the user's `AlexbeatsZ/openclaw` fork checkout on branch `fix/qa-lab-private-sdk-build`; push fork work to `origin` and use `upstream` only for synchronization unless an upstream PR is explicitly requested.
- The private QA cold-start fix added the missing `qa-channel` and `qa-channel-protocol` build entries and was deployed and verified before this state was compacted.
- Remaining validation gaps are Windows-local `test:changed` path casing and `check:changed` prompt snapshot/crabbox failures. Reproduce them against the current branch before attempting a fix.
