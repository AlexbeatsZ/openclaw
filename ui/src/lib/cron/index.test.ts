...
describe("cron controller", () => {
  ...
  it("normalizes main-session program delivery to announce", () => {
    const normalized = normalizeCronFormState({
      ...DEFAULT_CRON_FORM,
      sessionTarget: "main",
  ...
  it("requires an explicit channel and recipient for main-session program delivery", () => {
    ...
    expect(errors.deliveryChannel).toBe("cron.errors.directChannelRequired");
    expect(errors.deliveryTo).toBe("cron.errors.directToRequired");
  });

  it("forwards main-session program delivery in cron.add payload", async () => {
    const request = vi.fn(async (method: string) => {
      if (method === "cron.add") {
        return { id: "job-direct" };
      }
      if (method === "cron.list") {
        return { jobs: [] };
      }
    ...
    const state = createState({
      ...
      cronForm: {
        ...
        deliveryTo: "group:123",
        deliveryBestEffort: true,
      },
    });

    expect((await addCronJob(state)).saved).toBe(true);
    const addCall = findRequestCall(request.mock.calls, "cron.add");
    expectNestedRecordFields(requestPayload(addCall), "delivery", {
      mode: "announce",
      strategy: "direct",
      channel: "qqbot",
      to: "group:123",
