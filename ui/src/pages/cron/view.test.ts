...
describe("cron view editor", () => {
  ...
  it("renders main-session program delivery with explicit routing controls", () => {
    const container = renderView({
      createOpen: true,
      ...
      form: {
        ...
        deliveryMode: "announce",
        deliveryChannel: "qqbot",
      },
    });

    const deliveryMode = getElement(container, "#cron-delivery-mode", HTMLSelectElement);
    const deliveryChannel = getElement(container, "#cron-delivery-channel", HTMLSelectElement);
    expect(deliveryMode.disabled).toBe(true);
    expect(deliveryMode.value).toBe("announce");
    expect(
      Array.from(deliveryChannel.querySelectorAll("option"), (option) => option.value),
    ).toEqual(["qqbot", "telegram"]);
    expect(container.querySelector("#cron-delivery-to")).not.toBeNull();
    expect(container.textContent).toContain("Program delivery");
  });
