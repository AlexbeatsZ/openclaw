              ...
              >
                <button
                  ...
                  class="cfg-segmented__btn ${props.form.deliveryStrategy === "heartbeat"
                    ? "active"
                    : ""}"
                  @click=${() => props.onFormChange({ deliveryStrategy: "heartbeat" })}
                >
      ...
      ${renderFieldRow({
        ...
        control: html`
          <select
            ...
            ?disabled=${props.form.sessionTarget === "main" &&
            props.form.deliveryStrategy === "direct"}
            @change=${(e: Event) =>
              props.onFormChange({
      ...
      ${ctx.selectedDeliveryMode === "announce"
        ? html`
            ${renderFieldRow({
              ...
              required:
                props.form.sessionTarget === "main" && props.form.deliveryStrategy === "direct",
              error: props.fieldErrors.deliveryChannel,
              errorId: errorIdForField("deliveryChannel"),
              control: html`
                <select
                  id="cron-delivery-channel"
                  class="settings-select"
                  .value=${props.form.deliveryChannel || "last"}
                  aria-invalid=${props.fieldErrors.deliveryChannel ? "true" : "false"}
                  aria-describedby=${ifDefined(
                ...
                >
                  ${(props.form.sessionTarget === "main" && props.form.deliveryStrategy === "direct"
                    ? channelOptions.filter((channel) => channel !== "last")
                    : channelOptions
                  ).map(
                    (channel) =>
